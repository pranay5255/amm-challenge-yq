...
/// @title yq-v2: Original yq + sigma momentum for faster vol spike response
/// @notice When sigma is rising, add a momentum boost to react before the EMA catches up
contract Strategy is AMMStrategyBase {
    // --- decay / update constants (identical to yq) ---
    uint256 constant ELAPSED_CAP = 8;
    uint256 constant SIGNAL_THRESHOLD = WAD / 500;
    uint256 constant DIR_DECAY = 800000000000000000;
    ...
    uint256 constant SIGMA_DECAY = 824000000000000000;
    uint256 constant LAMBDA_DECAY = 990000000000000000;
    uint256 constant SIZE_BLEND_DECAY = 818000000000000000;
    uint256 constant TOX_BLEND_DECAY = 51000000000000000;
    uint256 constant ACT_BLEND_DECAY = 985000000000000000;
    ...
    uint256 constant MIN_GATE = 30000000000000000;
    ...
    uint256 constant STALE_ATTRACT_FRAC = 1124000000000000000;
    ...
    // --- fee model constants (identical to yq) ---
    ...
    uint256 constant TAIL_SLOPE_PROTECT = 930000000000000000;
    uint256 constant TAIL_SLOPE_ATTRACT = 955000000000000000;

    // =======================================================
    // NEW: Sigma momentum — boost fees when vol is rising
    // =======================================================
    // When sigma rises (sigmaHat > prevSigma), add momentum term
    // to react faster than the EMA alone
    uint256 constant SIGMA_MOMENTUM_COEF = 200000000000000000; // 0.20 WAD
    ...
    // slots[11] = prevSigma (NEW — sigma from previous step)
    ...
    function afterInitialize(uint256 initialX, uint256 initialY) external override returns (uint256, uint256) {
        ...
        slots[9] = 0;
        slots[10] = 0;
        slots[11] = 950000000000000; // prevSigma = initial sigma
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (isNewStep) {
            ...
            // Snapshot sigma before decay for momentum calculation
            prevSigma = sigmaHat;
        ...
        // Sigma momentum — boost fees when vol is rising
        if (sigmaHat > prevSigma) {
            uint256 sigmaDelta = sigmaHat - prevSigma;
            fMid = fMid + wmul(SIGMA_MOMENTUM_COEF, sigmaDelta);
        }
